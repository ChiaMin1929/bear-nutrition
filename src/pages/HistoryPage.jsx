    ...
    <div className="min-h-dvh bg-bg pb-safe">
      <div className="px-4 pt-5 pb-4">
        ...
        <div className="flex flex-col gap-2">
          {days.map(dateKey => {
            const log = getHistoryLog(dateKey)
              ...
              <div key={dateKey} className="bg-white rounded-2xl overflow-hidden shadow-sm">
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full ${isToday ? 'bg-coral' : hasData ? 'bg-mint' : 'bg-border'}`} />
                  ...
                  {totals && goals && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-text/40">{hasData ? '▾' : ''}</span>
                    </div>
                  )}
                </button>
                  ...
                  <div className="border-t border-border px-4 py-3 flex flex-col gap-2 fade-in">
                    ...
                    <div className="grid grid-cols-4 gap-2 mb-2">
                      {[
                        ...
                        { label: '蛋白質', val: log.entries.reduce((a,e) => a + (e.protein||0), 0), unit: 'g', color: 'text-teal' },
                        { label: '脂肪', val: log.entries.reduce((a,e) => a + (e.fat||0), 0), unit: 'g', color: 'text-yellow-500' },
                        { label: '碳水', val: log.entries.reduce((a,e) => a + (e.carbs||0), 0), unit: 'g', color: 'text-purple-400' },
                      ].map(m => (
                    ...
                    {log.entries.map(e => (
                      <div key={e.id} className="flex items-center justify-between py-1.5 border-b border-border/50 last:border-0">
                        <p className="text-sm text-text truncate max-w-[60%]">{e.food_name || e.name}</p>
